...
seat = "Comp. Engg.",
  ...
  100% {
    transform: translateZ(0);
        ...
        @keyframes ticket-color {
          0%          { background-color: #6366F1; } /* Indigo 500 */
10%         { background-color: #4F46E5; } /* Indigo 600 */
15%,25%     { background-color: #2563EB; } /* Blue 600 */
35%         { background-color: #3B82F6; } /* Blue 500 */
40%         { background-color: #6366F1; }
50%         { background-color: #4F46E5; }
55%,75%     { background-color: #2563EB; }
100%        { background-color: #6366F1; }
        }
        .ticket-card {
          animation: ticket-float 10s infinite;
        }
        .ticket-card:hover {
          animation-play-state: paused;
        }
        .ticket-card:hover .ticket-sheen::after,
        .ticket-card:hover .ticket-sheen::before,
        .ticket-card:hover .ticket-icons {
          animation-play-state: paused;
        }
        .ticket-sheen::after {
         .ticket-sheen::after {
  ...
  filter: blur(12px);
        ...
        .ticket-sheen::before {
          ...
          width: 80px;
          height: 100%;
          animation: ticket-light 10s infinite;
        }
        .ticket-icons {
          animation: ticket-color 10s infinite;
        }
      `}</style>

      <div className="ticket-card group relative z-10 flex h-[150px] w-[320px] gap-4 overflow-hidden rounded-2xl bg-white text-[#2d2d2d] shadow-lg transition-all duration-300 ease-[cubic-bezier(0.175,0.885,0.32,1.25)] hover:-translate-y-7">
        {/* Barcode */}
        <svg
          className="h-[150px] w-16 flex-shrink-0 fill-black"
        ...
        {/* Dashed separator */}
        <div className="pointer-events-none absolute left-[50px] top-0 z-10 flex h-full w-4 flex-col items-center justify-center">
          <span className="relative flex h-full border-l-2 border-dashed border-[#e8e8e8] before:absolute before:-top-2 before:left-1/2 before:h-4 before:w-4 before:-translate-x-1/2 before:rounded-full before:bg-[#e8e8e8] after:absolute after:-bottom-2 after:left-1/2 after:h-4 after:w-4 after:-translate-x-1/2 after:rounded-2xl after:bg-[#e8e8e8]" />
        </div>

        {/* Right content */}
        <div className="ticket-sheen relative flex w-full justify-between">
          <div className="flex w-full flex-col justify-between gap-1 px-3 py-2.5">
            {/* Route */}
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                <p className="text-[10px] leading-none text-[#aeaeae]">{from.country}</p>
                <p className="font-bold text-[#2d2d2d]">{from.code}</p>
                <p className="flex items-center gap-1 text-[10px] leading-3 text-[#2d2d2d]">
              ...
              <div className="flex flex-col">
                <p className="text-[10px] leading-none text-[#aeaeae]">{to.country}</p>
                <p className="font-bold text-[#2d2d2d]">{to.code}</p>
                <p className="flex items-center gap-1 text-[10px] leading-3 text-[#2d2d2d]">
                  <svg width="10" height="10" viewBox="0 0 1024 1024">
                    <path
                      fill="currentColor"
            ...
            <div className="border-b-2 border-[#e8e8e8]" />

            {/* Details */}
            <div className="flex flex-col gap-1 pb-1">
              <div className="flex w-full justify-between gap-1">
                <div className="text-[10px] leading-3">
                  <p className="text-[#aeaeae]">ID</p>
                  <p className="font-medium text-[#212121]">{id}</p>
                </div>
                <div className="text-right text-[10px] leading-3">
                  <p className="text-[#aeaeae]">Passenger</p>
                  <p className="font-medium text-[#212121]">{passenger}</p>
                </div>
              </div>
              <div className="flex w-full justify-between gap-1">
                <div className="text-[10px] leading-3">
                  <p className="text-[#aeaeae]">Flight</p>
                  <p className="font-medium text-[#212121]">{flight}</p>
                </div>
                <div className="text-[10px] leading-3">
                  <p className="text-[#aeaeae]">Gate</p>
                  <p className="font-medium text-[#212121]">{gate}</p>
                </div>
                <div className="text-[10px] leading-3">
                  <p className="text-[#aeaeae]">Seat</p>
                  <p className="font-medium text-[#212121]">{seat}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Icon strip */}
          <div className="ticket-icons z-[-1] flex w-fit flex-col items-center justify-between gap-3 p-2.5">
